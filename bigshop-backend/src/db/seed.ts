 ...
 * =============================================================================
 * Этот скрипт очищает БД и заполняет её демонстрационными данными
 * для разработки и тестирования приложения.
 * 
 * Выполняется командой: bun run db:seed
 * 
 * Создаёт:
 * - Категории товаров (фрукты, молочные, мясо, хлеб)
 * - Продукты с реальными изображениями из Unsplash
 * - Тестового пользователя для демонстрации
 */
 ...
 * 
 * Этапы:
 * 1. Очистка существующих данных (в правильном порядке из-за foreign keys)
 * 2. Создание категорий товаров
 * 3. Создание товаров с привязкой к категориям
...
async function seed() {
  try {
    ...
    // Сначала продукты (ссылаются на категории), затем категории, затем пользователи
